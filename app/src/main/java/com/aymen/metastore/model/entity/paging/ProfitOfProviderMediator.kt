...
class ProfitOfProviderMediator(
        ...
        return try {
            ...
            room.withTransaction {
                try {
                    ...
                    paymentForProvidersDao.insertProviderProfitHistoryKeys(response.map { article ->
                    ...
                    paymentForProvidersDao.insertPaymentForProviders(response.map {category -> category.toPaymentForProviders() })
    ...
    private suspend fun getPreviousPageForTheFirstItem(state: PagingState<Int, PaymentForProvidersWithCommandLine>): Int? {
        ...
        val remoteKey = entity?.let { paymentForProvidersDao.getProvidersProfitHistoryRemoteKey(it.paymentForProviders.id!!) }
