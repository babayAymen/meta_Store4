...
class NotAcceptedRemoteMediator(
        ...
        return try {
            ...
            room.withTransaction {
                try {
                    ...
                    invoiceDao.insertBuyHistoryNotAcceptedKeys(response.map { article ->
                        NotAcceptedRemoteKeysEntity(
                            id = article.id!!,
                            nextPage = nextPage,
                            prevPage = prevPage,
                        )
    ...
    private suspend fun getPreviousPageForTheFirstItem(state: PagingState<Int, InvoiceWithClientPersonProvider>): Int? {
        ...
        val remoteKey = entity?.let { invoiceDao.getBuyHistoryNotAcceptedRemoteKey(it.invoice.id!!) }
    ...
    private suspend fun getNextPageClosestToCurrentPosition(state: PagingState<Int, InvoiceWithClientPersonProvider>): Int? {
        ...
        val remoteKey = entity?.invoice?.id?.let { invoiceDao.getBuyHistoryNotAcceptedRemoteKey(it) }
        return remoteKey?.nextPage
    }
